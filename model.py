...
class Net(nn.Module):
    ...
    def forward(self, *input):
        ...
        # node_feature = torch.from_numpy(np.array(range(1, self.poi_size))).cuda()
        # node_feature = self.embed(node_feature)
        # h = self.conv4(node_feature, topology)
        # h = F.relu(h)
        # h = F.dropout(h, training=self.training)
        # print(h.shape)
        # after_graph_embed = nn.Embedding(self.poi_size+1, self.hidden_dim, padding_idx=0).cuda()
        # after_graph_embed.weight.data.copy_(h)

        user_history = self.bat_nor_embed(self.embed(user).transpose(1, 2).contiguous()).transpose(1, 2)
        poi_preference = self.bat_nor_embed(self.embed(poi).transpose(1, 2).contiguous()).transpose(1, 2)

        user_history = torch.nn.utils.rnn.pack_padded_sequence(user_history, length.cpu().tolist(), batch_first=True)
        user_preference, (h_n, c_n) = self.lstm(user_history) 
        user_preference = h_n.transpose(0, 1).contiguous()
        # user_preference_pad, out_len = torch.nn.utils.rnn.pad_packed_sequence(user_preference, batch_first=True)

        c_preference = torch.cat([user_preference, poi_preference], -1)
        c_preference = torch.squeeze(c_preference, 1)
        output = self.fc(c_preference)
...
attribute_embedding_matrix = process.create_attribute_matrix(Parameters.shopMes_path)
pre_trained_embedding_matrix = process.create_embedding_matrix(type_embedding_matrix, attribute_embedding_matrix)
user_history_data_dict = process.split_review_data(Parameters.review_path, shop_index)
struct_matrix = process.construct_graph(Parameters.shopMes_path, shop_index)

net = Net(poi_size=len(shop_index), embedding_matrix=pre_trained_embedding_matrix, struct_topology=struct_matrix, user_history_data_dict=user_history_data_dict).cuda()
net.fit("model.pkl")
