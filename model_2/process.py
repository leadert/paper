...
def split_review_data(review_path, shop_index):
    ...
    for user_id, group in grouped:
        ...
        if user_count < Parameters.history_review_count:         # 过滤少于k个历史数据的用户
            continue
        else:
            df_train_data = group[:int(group.shape[0])-1]
            df_test_data = group[int(group.shape[0])-1:]      # 最后一条作为测试集
...
def construct_graph(shopMes_path, shop_index):
    ...
    struct_list = []
    df = pd.read_csv(shopMes_path, low_memory=False)
    df_copy = df
    for index, row in df.iterrows():
        poi_id_1 = row['shopId']
...
def yield_train_data(data_path, user_history_data_dict, max_len, shop_index, batch_size):
    ...
    for line in f:
        if line_count == 0:
            ...
            user = user_history_data_dict[int(line[0])]       # user历史数据list, int:list
            if str(line[2]) in shop_index.keys():
                if len(user) > max_len:
                    user = user[-max_len:]
                    length_index.append(max_len)
                    poi = [shop_index[str(line[2])]]
                else:
                ...
                label.append(int(float(line[4])) - 1)
                count += 1
...
def get_test_data(data_path, user_history_data_dict, max_len, shop_index, shop_location_dict):
    ...
    for line in f:
        if line_count == 0:
            ...
            line = line.strip().split(",")
            if line[2] in shop_index:
                user = user_history_data_dict[int(line[0])]  # user历史数据list, int:list
                # if len(user) > max_len:
                #     user = user[-max_len:]
                # else:
                #     user = user + [0] * (max_len - len(user))
                poi = [shop_index[line[2]]]  # str : str
                location = [shop_location_dict[int(line[2])]]
...
# shop_location_dict = create_POI_index(Parameters.shopMes_path, Parameters.shop_index_path)
# shop_index = load_POI_index(Parameters.shop_index_path)
# poincareModel = PoincareModel.load(Parameters.hieraechical_path)
# type_embedding_matrix = create_type_matrix(Parameters.shopMes_path, poincareModel, shop_index)
# user_history_data_dict = split_review_data(Parameters.review_path, shop_index)
# # struct_matrix = construct_graph(Parameters.shopMes_path, shop_index)      # 邻接POI
